...
def test_post_v1_account():
    ...
    responce = account_api.post_v1_account(json_data=json_data)
    ...
    print(response.status_code)
    print(response.text)
    assert responce.status_code == 201, f"user not activated: {responce.status_code}"
    # авторизоваться
    json_data = {
        'login': login,
        'password': password,
    ...
    print(response.status_code)
    print(response.text)
    assert responce.status_code == 201, f"user can't authorize: {responce.status_code}"
